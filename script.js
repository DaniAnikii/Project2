...
var contractAddress = '0x75A2D9B2eD5919A53CC05f7F2256aA196539D995'; // FIXME: fill this in with your contract's address/hash
...
async function getTotalOwed(user) {
	...
	const listIOUs = await BlockchainSplitwise.methods.getIOUsArray().call();
	for(var i=0; i<listIOUs.length; i++){
		if (listIOUs[i].debtor == user){
			amount += parseInt(listIOUs[i].amount);
		}
		if (listIOUs[i].creditor == user){
			amount -= parseINt(listIOUs[i].amount);
		}
	}
	return amount;
...
async function getLastActive(user) {
	...
	const listIOUs = await BlockchainSplitwise.methods.getIOUsArray().call();
	for(var i=0; i<listIOUs.length; i++){
		if (listIOUs[i].debtor == user || listIOUs[i].creditor == user){
			if(listIOUs[i].date > lastDate){
				lastDate = listIOUs[i].date;
			}	
		}
	}
	return lastDate;
